...
class Tank_auth
{
	...
	function set_temp_session($task)
	{
		$this -> ci -> session -> set_userdata(array(
								'temp_session_'.$task => true
						));
	}
	 ...
	 * @return	string
	 */
	function get_temp_session_status($task)
	{
		return $this -> ci -> session -> userdata('temp_session_'.$task);
	}
	 ...
	 * @return	array
	 */
	function create_user($username, $user_full_name, $user_bn_name,$designation, $user_address, $status, $contact, $web, $biodata, $is_publisher,$email, $password, $user_type, $email_activation)
	{
	...
	// public function edit_data($id, $new_pass){
	...
	function change_user_password($id, $new_pass){
		$hasher = new PasswordHash(
			$this->ci->config->item('phpass_hash_strength', 'tank_auth'),
			$this->ci->config->item('phpass_hash_portable', 'tank_auth')
		);
